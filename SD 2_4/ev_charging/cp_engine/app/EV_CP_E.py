...
class EV_CP_E:
    ...
    def cifrar_mensaje_completo(self, mensaje_completo):
        ...
        secret_key = self.obtener_clave_secreta()
        cp_id_visible = mensaje_completo.get('cp_id')
        
        if not secret_key or not cp_id_visible:
            print("⚠️ Advertencia: Falta clave secreta o 'cp_id'. El mensaje no pudo ser cifrado")
            return None
        ...
        try:
            ...
            return None # Retornar el mensaje original como fallback

    def descifrar_mensaje_completo(self, mensaje_kafka):
        ...
        # 2. Obtener la Clave
        clave_secreta = self.obtener_clave_secreta()
    ...
    def suministrar_energia(self):
        ...
        try: # Bucle de suministro
            while not self.parar_suministro.is_set(): # Mientras no se haya solicitado parar
                ...
                mensaje = self.cifrar_mensaje_completo(mensaje)

                if mensaje is None:
                    print("❌ Error: No se pudo cifrar el mensaje de suministro. Suministro detenido.")
                    break
            ...
            mensaje_final = self.cifrar_mensaje_completo(mensaje_final)

            if mensaje_final is None:
                print("❌ Error: No se pudo cifrar el mensaje final de suministro.")
                return
    ...
    def responder_menu(self, switch):
        ...
        if switch == "1": #Mostrar estado actual
            ...
            if self.estado == MENSAJES_CP_M.STATUS_OK.value and not self.suministrar_actvio: # Si está OK y no está suministrando
                ...
                mensaje_inicio = self.cifrar_mensaje_completo(mensaje_inicio)

                if mensaje_inicio is None:
                    print("❌ Error: No se pudo cifrar el mensaje de inicio. Operación cancelada.")
                    return
                ...
                mensaje_stop = self.cifrar_mensaje_completo(mensaje_stop)

                if mensaje_stop is None:
                    print("❌ Error: No se pudo cifrar el mensaje de parada. Operación cancelada.")
                    return
