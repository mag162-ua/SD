...
class DatabaseManager:
    ...
    def create_new_secret_key(self, cp_id) -> Optional[str]:
...
class RealKafkaManager:
    ...
    def get_new_secret_key(self, cp_id):
        try:
            with open(f"/app/central_claves/clave_{cp_id}.json", "r") as archivo:
                estado_info = json.load(archivo)
                #self.secret_key = estado_info.get("secret_key", None)
                return estado_info.get("secret_key", None)
        except Exception as e:
    ...
    def cifrar_mensaje_completo(self, mensaje_completo):
        ...
        if not secret_key or not cp_id_visible:
            print("⚠️ Advertencia: Falta clave secreta o 'cp_id'. Se envía sin cifrar.")
            return mensaje_completo

        try:
            payload_a_cifrar = mensaje_completo.copy()
    ...
    def descifrar_mensaje_completo(self, mensaje_kafka: dict):
        ...
        #mensaje_kafka = json.loads(mensaje_kafka)

        if isinstance(mensaje_kafka, str):
            try:
                mensaje_kafka = json.loads(mensaje_kafka)
            except Exception as e:
                print(f"❌ Error al parsear mensaje Kafka: {e}")
                return None

        cp_id = mensaje_kafka.get('cp_id')
    ...
    def send_message(self, topic: str, message: dict):
        ...
        try:
            if self.producer:
                if 'driver' not in topic:
                    message = self.cifrar_mensaje_completo(message)
                    if message is None:
                        logger.error("❌ No se pudo cifrar el mensaje. No se envía.")
                        return
                ...
                for message in message_batch:
                    if message.value:
                        try:
                            ...
                            if 'driver' not in topic:
                                message_dict = self.descifrar_mensaje_completo(message_dict)
                                if message_dict is None:
                                    logger.error("❌ No se pudo descifrar el mensaje recibido.")
                                    continue
                            messages.append({
                            ...
                            logger.error(f"❌ Error procesando mensaje por kafka: {e}")
...
class SocketServer:
    ...
    def process_message(self, message: str, client_socket):
        ...
        try:
            ...
            logger.error(f"❌ Error procesando mensaje por socket: {e}", exc_info=True)  
    ...
    def handle_cp_registration(self, params: List[str], client_socket):
        ...
        new_secret_key = self.central.database.create_new_secret_key(cp_id)
...
class EVCentral:
    ...
    def process_control_command(self, message):
        ...
        try:
            ...
            real_ip = message.get('source_ip', source)
    ...
    def process_supply_response(self, message):
        ...
        try:
            ...
            source_ip = message.get('source_ip', 'Engine-Unknown')
            ...
            if message_type == 'SUPPLY_REQUEST':
                ...
                if cp.status == "SUMINISTRANDO":
                    ...
                    # Actualizar estado
                    self.update_cp_status(cp_id, "ACTIVADO", source_ip=source_ip)
                else:
    ...
    def process_driver_request(self, message):
        ...
        try:
            ...
            source_ip = message.get('source_ip', 'App-Unknown')
            ...
            if request_type == 'SUPPLY_REQUEST':
                if not cp_id:
                    ...
                    return
                self.handle_driver_supply_request(driver_id, cp_id, source_ip)
                ...
                self.handle_driver_cancel_supply(driver_id, cp_id, message.get('reason', 'Cancelación del conductor'), source_ip)
                self.update_cp_status(cp_id, "ACTIVADO")
    ...
    def handle_driver_cancel_supply(self, driver_id: str, cp_id: str, reason: str, source_ip: str = None):
        ...
        try:
            ...
            if cp.current_amount > 0.01:
                transaction_data = self.record_transaction(cp, "CANCELLED", source_ip=source_ip)
                if transaction_data:
    ...
    def handle_driver_supply_request(self, driver_id: str, cp_id: str, source_ip: str = None):
        ...
        if cp and cp.status == "ACTIVADO":
            if self.authorize_cp_supply(cp_id, driver_id, source_ip):
    ...
    def authorize_cp_supply(self, cp_id: str, driver_id: str, source_ip: str = None) -> bool:
        ...
        try:
            ...
            if cp.status != "PARADO":
                self.update_cp_status(cp_id, "SUMINISTRANDO", driver_id=driver_id, source_ip=source_ip)
