...
docker compose up -d cp_engine --scale cp_engine=3
...
for /l %%i in (1,1,%NUMERO_INSTANCIAS%) do (
    REM set /a PUERTO_E=%PUERTO_INI% + %%i - 1
    set PUERTO_E=6000
    echo Ejecutando CP Engine %%i en puerto !PUERTO_E!...
    start "Engine %%i" cmd /k docker exec -it ev_charging-cp_engine-%%i python EV_CP_E.py kafka:9092 !PUERTO_E!
)
